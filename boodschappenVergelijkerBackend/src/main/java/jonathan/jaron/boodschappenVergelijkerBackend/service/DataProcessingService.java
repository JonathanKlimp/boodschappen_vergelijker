...
public class DataProcessingService {
    ...
    public void processAndSaveData(File jsonData) throws IOException {
        ...
        List<SupermarktDto> supermarkten = objectMapper.readValue(jsonData, new TypeReference<List<SupermarktDto>>() {
        });
        System.out.println("processing and saving data");
        for (SupermarktDto supermarktDto : supermarkten) {
            for (ProductDto productDto : supermarktDto.getProducten()) {
                productDto.setSupermarkt(supermarktDto);
            }
            System.out.println("saving data van: " + supermarktDto.getNaam());
            supermarktRepository.save(supermarktDto);

            productRepository.saveAll(supermarktDto.getProducten());
        }
